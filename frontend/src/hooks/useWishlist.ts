...
import {
  userWishlistStatusQueryKey,
  userWishlistsQueryKey,
  userWishlistsQueryKeyPrefix,
...
export function useWishlistStatus(bookId: number, enabled: boolean) {
  return useQuery<boolean>({
    queryKey: userWishlistStatusQueryKey(bookId),
...
export function useWishlistMutation(bookId: number, source: string = "none") {
  ...
  const statusKey = userWishlistStatusQueryKey(bookId);
  ...
  const addMutation = useMutation({
    ...
    onSettled: () => {
      ...
      queryClient.invalidateQueries({ queryKey: userWishlistsQueryKeyPrefix() });
    },
  });
...
export function useMyWishlists(
  ...
  return useQuery<WishlistPageData, Error>({
    queryKey: userWishlistsQueryKey(page, size),
